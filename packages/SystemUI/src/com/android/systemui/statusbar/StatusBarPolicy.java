...
public class StatusBarPolicy {
    ...
    private static final int EVENT_UPDATE_CDMA_SIGNAL = 5;
    ...
    private int mOldcdmalevel = 0; //added by zhangbo
...
//        }
        ...
        int batteryId = setBatteryStyle(batteryStatus);
        mService.setIcon("battery", batteryId, level);
        
        mBatteryStatus = batteryStatus;
//        if (mStatusBarBattery == BATTERY_STYLE_NORMAL) {
//               mService.setIconVisibility("battery", true);
//           mService.setIcon("battery", com.android.internal.R.drawable.stat_sys_battery2, level);
//       } else if (mStatusBarBattery == BATTERY_STYLE_PERCENT || mStatusBarBattery == BATTERY_STYLE_GONE) {
//               mService.setIconVisibility("battery", false);
//           mService.setIcon("battery", com.android.internal.R.drawable.stat_sys_battery, level);
//       }  
    ...
    private int setBatteryStyle(int status) {
        int i = Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.STATUS_BAR_BATTERY_STYLE, 0);

        if (status == BatteryManager.BATTERY_STATUS_CHARGING) {
            if(i == BATTERY_STYLE_PERCENT) {
                return com.android.internal.R.drawable.stat_sys_battery2_charge;
            }
            ...
            if(i == BATTERY_STYLE_PERCENT) {
                return com.android.internal.R.drawable.stat_sys_battery2;
            }
            return com.android.internal.R.drawable.stat_sys_battery;
        } else {
            if(i == BATTERY_STYLE_PERCENT) {
                return com.android.internal.R.drawable.stat_sys_battery2_unknown;
            }
    ...
    private void updateConnectivity(Intent intent) {
        ...
        int inetCondition = 0;//modify by zhangbo for icon style
        switch (info.getType()) {
        case ConnectivityManager.TYPE_MOBILE:
            mInetCondition = inetCondition;
            Slog.d(TAG, "updateConnectivity info.getSubtype() = " +info.getSubtype());//added by zhangbo
    ...
    private void scheduleUpdatecdmaSignal() {
        Message m = mHandler.obtainMessage(EVENT_UPDATE_CDMA_SIGNAL);
        //m.arg1 = (++mBatteryViewSequence);
    ...
    private final void updateSignalStrength() {
        ...
        if (!isCdma()) {
            ...
            if (SystemProperties.get("ro.ril.samsung_cdma").equals("true")) {
                ...
                //modify by zhangbo 1205
//                Slog.d(TAG, "mPhoneState=" + mPhoneState + ", isEvdo=" + isEvdo() + ", mAlwaysUseCdmaRssi=" + mAlwaysUseCdmaRssi
//                        + ", mOldcdmalevel=" + mOldcdmalevel);

                if ((mPhoneState == TelephonyManager.CALL_STATE_IDLE) && isEvdo()
                    && !mAlwaysUseCdmaRssi) {
                    iconLevel = getEvdoLevel();
                    if (true) {
//                        Slog.d(TAG, "1 use Evdo level=" + iconLevel + " to replace Cdma Level=" + getCdmaLevel());
                    }
                } else {
                    if ((mPhoneState == TelephonyManager.CALL_STATE_IDLE) && isEvdo()){
                        iconLevel = getEvdoLevel();
                        if (true) {
                            Slog.d(TAG, "2 use Evdo level=" + iconLevel + " to replace Cdma Level=" + getCdmaLevel());
                        }
                    } else {
                        iconLevel = getCdmaLevel();
                    }
                ...
                if (mOldcdmalevel >= iconLevel) {
                    //add
                    if (!mHandler.hasMessages(EVENT_UPDATE_CDMA_SIGNAL) && mOldcdmalevel != iconLevel) {
                        mOldcdmalevel--;
                        scheduleUpdatecdmaSignal();
                    }
                    return ; 
                } else {
                    //remove ---
                    if (mHandler.hasMessages(EVENT_UPDATE_CDMA_SIGNAL)) {
                        mHandler.removeMessages(EVENT_UPDATE_CDMA_SIGNAL);
                    }                   
                    mOldcdmalevel = iconLevel;
                }
                mPhoneSignalIconId = iconList[mOldcdmalevel];
                mService.setIcon("phone_signal", mPhoneSignalIconId, 0);
                return ;
            }
        }
    ...
    private int getCdmaLevel() {
        ...
        levelCdma = (levelDbm < levelEcio) ? levelDbm : levelEcio;
        if ((levelCdma != 0) && (levelCdma < 4)) {
            levelCdma += 1;
        }
        return levelCdma;
    ...
    private int getEvdoLevel() {
        ...
        levelEvdo = (levelEvdoDbm < levelEvdoSnr) ? levelEvdoDbm : levelEvdoSnr;
        if ((levelEvdo != 0) && (levelEvdo < 4)) {
            levelEvdo += 1;
        }
        return levelEvdo;
    ...
    private final void updateVolume() {
        ...
        final boolean visible = ringerMode == AudioManager.RINGER_MODE_SILENT ||
                ...
                vibrateMode == AudioManager.VIBRATE_SETTING_ONLY_SILENT;
                ;
        ...
        {
            if(vibrateMode == AudioManager.VIBRATE_SETTING_ONLY_SILENT )
            {
                //iconId = R.drawable.stat_sys_ringer_silent_vibrate;
            }
            else if(vibrateMode == AudioManager.VIBRATE_SETTING_ON)
            {
    ...
    private class StatusBarHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            case EVENT_BATTERY_CLOSE:
                ...
                break;
            case EVENT_UPDATE_CDMA_SIGNAL:
                {
                    Slog.d(TAG, "EVENT_UPDATE_CDMA_SIGNAL   mOldcdmalevel=" + mOldcdmalevel);
                    mPhoneSignalIconId = sSignalImages[mInetCondition][mOldcdmalevel];
                    mService.setIcon("phone_signal", mPhoneSignalIconId, 0);
                }
                break;
            }
...
//        if (mStatusBarBattery == BATTERY_STYLE_NORMAL) {
//             //   mService.setIconVisibility("battery", true);
//            mService.setIcon("battery", com.android.internal.R.drawable.stat_sys_battery2, mBatteryLevel);
//        } else if (mStatusBarBattery == BATTERY_STYLE_PERCENT || mStatusBarBattery == BATTERY_STYLE_GONE) {
//             //   mService.setIconVisibility("battery", false);
//            mService.setIcon("battery", com.android.internal.R.drawable.stat_sys_battery, mBatteryLevel);
//        }
        
        int batteryId = setBatteryStyle(mBatteryStatus);
        
        mService.setIcon("battery", batteryId, mBatteryLevel);
