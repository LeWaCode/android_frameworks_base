...
public class LewaSmsSelector extends FrameLayout implements AnimationListener{
    
    private OnSmsDialTriggerListener mOnDialTriggerListener;
	...
	private static String TAG = "LewaSmsSelector";
	...
	private ImageView new_msg_bg = null;

	private float mDensity;
	public LewaSmsSelector(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
	...
	public LewaSmsSelector(Context context) {
		super(context);
		mContext = context;
		mScroller = new Scroller(context);
		mDensity = context.getResources().getDisplayMetrics().density;
	...
	public boolean onTouchEvent(MotionEvent event) {
        ...
        if (new_msg_bg == null) {
            new_msg_bg = (ImageView)this.findViewById(R.id.new_msg_bg);
        }
		...
		case MotionEvent.ACTION_DOWN:
            ...
            new_msg_bg.setBackgroundResource(R.drawable.lock_noti_bg_hi);
            break; 
		...
		case MotionEvent.ACTION_UP:
			...
			new_msg_bg.setBackgroundResource(R.drawable.lock_noti_bg);
			...
			if (LewaLockScreen_zb.mPluggedIn) {
                ...
                LewaLockScreen_zb.setUnlocktipsVisible(GONE);
                    ...
                    if (upInUnlockPosition(event.getRawY())) {		    
                        if(mLenseMode)
                            dispatchTriggerEvent(OnSmsDialTriggerListener.LEFT_HANDLE);
                        else
                            dispatchTriggerEvent(OnSmsDialTriggerListener.MID_HANDLE);

                        if(LewaLockScreen_zb.mUnreadMsgCount == 1) {
                            Uri uri = null;
                            if(LewaLockScreen_zb.mUnreadSmsCount == 1)
                            {
                                if(LewaLockScreen_zb.mUnreadSmsNum != null)
                                {
                                    uri = Uri.parse("smsto:" + LewaLockScreen_zb.mUnreadSmsNum);
                            ...
                            {
                                if(LewaLockScreen_zb.mUnreadMmsNum != null)
                                {
                                    uri = Uri.parse("smsto:" + LewaLockScreen_zb.mUnreadMmsNum);
                                    Intent intent = new Intent(Intent.ACTION_SENDTO, uri);
                                    intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                                    intent.putExtra("LockScreen_MSG", "true");
                                    mContext.startActivity(intent);
	...
	public void computeScroll() {
		...
		if (mScroller.computeScrollOffset()) { 
			log("getCurrY = "+mScroller.getCurrY());
    ...
    public void setOnSmsDialTriggerListener(OnSmsDialTriggerListener l) {
    ...
    private void setGrabbedState(int newState) {
        if (true/*newState != mGrabbedState*/) {
            ...
            if (mOnDialTriggerListener != null) {
                mOnDialTriggerListener.onSmsGrabbedStateChange(this, mGrabbedState);
            }
        }
    }
    ...
    private void dispatchTriggerEvent(int whichHandle) {
        //vibrate();
        if (mOnDialTriggerListener != null) {
            mOnDialTriggerListener.onSmsDialTrigger(this, whichHandle);
        }
    }
    
	public interface OnSmsDialTriggerListener {
        /**
         ...
         */
        void onSmsDialTrigger(View v, int whichHandle);
         ...
         */
        void onSmsGrabbedStateChange(View v, int grabbedState);
    }
